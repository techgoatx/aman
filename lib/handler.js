...
const { sameJid } = require('../permissions/identity.js');
...
function isBanned(jid) {
    try {
        ...
        return bannedJids.some(b => sameJid(jid, b) || jid === b);
    } catch (e) {
        return false;
    }
}
...
async function handler(sock, msg) {
    try {
        ...
        const isMasterUser = await permissions.master.isMaster(sender, sock, chatId);
        const isOwnerUser = await permissions.owner.isOwner(sender, sock, chatId);
        const isSudoUser = await permissions.sudo.isSudo(sender, sock, chatId);
        const isAdminUser = await permissions.admin.isAdminOrOwner(sock, sender, chatId);

        const permission = permissions.checkPermission(sender, command, botMode, isMasterUser, isOwnerUser, isSudoUser, isAdminUser);
